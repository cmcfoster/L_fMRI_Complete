...
w1cog <- read.xlsx(paste0(bp,'/KK_KR_JLBS/Wave1/Demographic_and_Task_Data/Cog_Data_W1.xlsx'), detectDates = T)
w1cog <- w1cog[,c('ID','SEX','AGE','ED_YEARS','CESD','MMSE', 'S1_DATE$')]
w1cog <- w1cog[!(is.na(w1cog$ID)),]
names(w1cog) <- c('CogID','SEX','CogAGE','ED_Years','CESD','MMSE', 'S1Date')

w2cog <- read.xlsx(paste0(bp,'/KK_KR_JLBS/Wave2/Demographic_and_Task_Data/Cog_Data_W2.xlsx'), detectDates = T)
w2cog <- w2cog[,c('ID','SEX','AGE_W2','ED_Years_W2','CESD_W2','MMSE_W2', 'S1_DATE_W2')]
w2cog <- w2cog[!(is.na(w2cog$ID)),]
names(w2cog) <- c('CogID','SEX','CogAGE','ED_Years','CESD','MMSE', "S1Date")
w2cog$S1Date <- as.Date(w2cog$S1Date)
...
w1full2$DJ_ScanDate2 <- as.Date(w1full2$DJ_ScanDate,format="%m/%d/%Y")
w1full2$Nback_ScanDate2 <- as.Date(w1full2$Nback_ScanDate,format="%m/%d/%Y")
...
w2full2$DJ_ScanDate2 <- as.Date(w2full2$DJ_ScanDate,format="%m/%d/%Y")
w2full2$Nback_ScanDate2 <- as.Date(w2full2$Nback_ScanDate,format="%m/%d/%Y")
...
# checking age match
w1age <- subset(w1full2,select = c(CogID, Age, CogAGE, DJAgeAtScan, NbackAgeAtScan, Bday2, S1Date, DJ_ScanDate2, Nback_ScanDate2))
w2age <- subset(w2full2,select = c(CogID, Age, CogAGE, DJAgeAtScan, NbackAgeAtScan, Bday2, S1Date, DJ_ScanDate2, Nback_ScanDate2))

w1age$AgeAtCog <- floor(as.numeric(as.period(interval(w1age$Bday2,w1age$S1Date)),'years'))
w2age$AgeAtCog <- floor(as.numeric(as.period(interval(w2age$Bday2,w2age$S1Date)),'years'))
