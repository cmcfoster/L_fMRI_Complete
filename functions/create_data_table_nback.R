...
files <- paste0("/raid/data/shared/KK_KR_JLBS/Longitudinal_Data/W1_W2/MRI/FMRI/data", "/", subj_list, "/first_level_nback/con_000") %>%
  rep(., each = 4) %>%
  paste0(., c(1:4), ".nii")
head(files)
...
df_nii <- tibble(id = NA,
  ...
  mutate(mri_id_start = str_locate(files, "3tb") %>% .[[1]],
         ...
         contrast = case_when(
           str_detect(file, "con_0001") ~ -2.25,
           str_detect(file, "con_0002") ~ -0.25,
           str_detect(file, "con_0003") ~ 0.75,
           str_detect(file, "con_0004") ~ 1.75,
         )) %>%
...
df_age_lag <- df_age_lag_orig %>%
  select(id = CogID, wave = Wave, mri_id = MRI_ID, age_w1 = DJAgeW1, lag_years = Lag, Excluded_from_all, excluded.from.nback) %>%
  filter(!is.na(age_w1),
         Excluded_from_all == 0,
         excluded.from.nback == 0,
         id %in% unique_id,
...
# save table ----
write_tsv(df_data_clean, "/Volumes/shared/KK_KR_JLBS/Longitudinal_Data/W1_W2/MRI/FMRI/group_analyses/nback/in_data_table.txt")  
